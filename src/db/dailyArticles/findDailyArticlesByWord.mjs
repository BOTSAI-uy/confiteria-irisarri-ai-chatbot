...
export async function findDailyArticlesByWord(word) {
  ...
  const articles = cacheArticles.filter((article) => {
    ...
    if (article.descripcion?.toLowerCase().includes(searchWord)) {
      return true
    }

    // Buscar en la descripción del artículo
    if (article.descripcionAvanzada?.toLowerCase().includes(searchWord)) {
      return true
    }

    // Buscar en el alias del artículo
    if (article.alias?.toLowerCase().includes(searchWord)) {
      return true
    }

    // Buscar en las familias
    if (article.familia?.toLowerCase().includes(searchWord)) {
      return true
    }

    // Buscar en el ramo
    if (article.ramo?.toLowerCase().includes(searchWord)) {
      return true
    }

    // Buscar en el grupo
    if (article.grupo?.toLowerCase().includes(searchWord)) {
      return true
    }
    return false
  })
